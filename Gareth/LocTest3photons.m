...
% Pj_zero = kron(kron(kron(Pj0,Pj0),I),I);
% Pj_one = kron(kron(kron(Pj1,Pj1),I),I);

Pj_zero = kron(kron(kron(Pj0,I),Pj0),I);
Pj_one = kron(kron(kron(Pj1,I),Pj1),I);

% Pj_zero = kron(kron(kron(I,I),Pj0),Pj0);
% Pj_one = kron(kron(kron(I,I),Pj1),Pj1);


Ps_zero = trace(Pj_zero*rho_cluster);
Ps_one = trace(Pj_one*rho_cluster);

% rho_reduced_zero = (1/Ps_zero) * PartialTrace(Pj_zero*Pj_zero*Pj_zero*rho_cluster,[1,2],[2,2,2,2],-1);
% rho_reduced_one = (1/Ps_one) * PartialTrace(Pj_one*Pj_one*Pj_one*rho_cluster,[1,2],[2,2,2,2],-1);

rho_reduced_zero = (1/Ps_zero) * ptrace(Pj_zero*rho_cluster,[2,3],[2,2,2,2]);
rho_reduced_one = (1/Ps_one) * ptrace(Pj_one*rho_cluster,[2,3],[2,2,2,2]);
